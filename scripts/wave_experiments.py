...
def gallery():
    ...
    for number, (slug, title, description, construction) in enumerate(VARIANTS, 1):
        ...
        manifest.append(dict(number=number, slug=slug, title=title, branch=f'codex/{slug}', revision=revision, worktree=str(project)))
...
<footer><span>{revision}</span><button class="copy" data-command="{html.escape(command, quote=True)}">Copy launch command</button></footer></article>''')
