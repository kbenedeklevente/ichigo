...
func _initialize() -> void:
	...
	_check(runtime.director.trigger("weather.mix.storm.strong"), "A progression trigger admits mixed weather.")
	...
	_check(runtime.director.snapshot().active.has(runtime.active_weather_id), "Arrival does not automatically finish the event.")
	...
	_check(runtime.director.snapshot().last_outcome.get("weather.mix.storm.strong") == "completed", "Completion follows the full weather lifecycle.")
	_check(not runtime.director.snapshot().active.has("weather.mix.storm.strong"), "Finished weather releases the exclusive group.")
	_check(not runtime.director.trigger("weather.mix.storm.strong"), "Finished weather respects its cooldown.")
	var first = _runtime()
	var second = _runtime()
	first.advance(8.0, Vector2.ZERO)
	for index in range(80):
		second.advance(0.1, Vector2.ZERO)
	_check(first.director.snapshot() == second.director.snapshot(), "Rendered frame partition does not change chance scheduling.")
